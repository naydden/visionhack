...
kp1, pass_lev = sift.detectAndCompute(pass_lev,None)

no_pass = []
for file in glob.glob("/home/bobz/repos/OpenCV-programs/visionhack/zebra/test_images/bad/*.png"):
	img = cv2.imread(file,0)
	no_pass.append(img)
kps, no_pass = sift.detectAndCompute(np.array(no_pass),None)
...
def SIFT(img2):
	...
	matches_np = flann.knnMatch(no_pass,des2,k=2)
	matches_lev = flann.knnMatch(pass_lev,des2,k=2)
	# store all the good matches as per Lowe's ratio test.
	good_np = []
	for m,n in matches_np:
	...
	if(size_lev > size_np):
		return (size_lev,'l')
	elif(size_np > size_lev):
		return (0, 'n')
	elif(size_np == size_lev):
		return (size_lev, 'l') if good_lev[0].distance < good_np[0].distance else (0,'n')
	else:
		return (0,'n')
zebra = False
level = False
